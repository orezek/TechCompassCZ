...
async function runIngestionPipeline<T extends Document>(mongoDbCollection: Collection<T>, csvFilePath: string, zodSchema: ZodType) {
  ...
  const csvParserTransformStream = parse({ headers: true, trim: true, ignoreEmpty: true });
  ...
  await pipeline(sourceFileReadStream, csvParserTransformStream, zodSchemaTransformStream, mongoDbWriteStream);
}
...
const CSV_FILE_PATH = "./sampleJobsCsv.csv";
await runIngestionPipeline(stagedCollection, CSV_FILE_PATH, stagedJobRecordsSchema);
