...
if (!cloudSearchJobCollection || !cloudEnrichedJobCollection)
...
const queryResult = await cloudSearchJobCollection
  .aggregate(mongoDbQuery)
  .toArray();
const parent = new ObjectId(queryResult[0]!.parentId);
const doc = await cloudEnrichedJobCollection.findOne({
  _id: queryResult[0]!.parentId,
});
...
export async function jobSearch(query: string) {
  ...
  const result = await cloudSearchJobCollection
    .aggregate(mongoDbQuery)
    .toArray();
  const index = randomInt(1, 15);
  const doc = await cloudEnrichedJobCollection.findOne({
    _id: result[index]!.parentId,
  });
