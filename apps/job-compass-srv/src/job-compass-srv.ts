...
const search = tool(async ({ query }) => jobSearch(query), {
  name: "jobSearch",
  ...
  schema: z.object({
    query: z
      .string()
      .describe(
        "The search query. Be very specific as the fields accepts semantic meaning.",
      ),
  }),
});
...
fastify.post("/threads/:thread_id/runs/stream", async (req, reply) => {
  ...
  try {
    ...
    fastify.log.info(
      { thread_id, hasInput: !!body.input },
      "Thread-specific stream request",
    );
...
const start = async () => {
  try {
    ...
    console.log(
      "🚀 LangGraph-compliant server running on http://localhost:3333",
    );
  } catch (err: unknown) {
    fastify.log.error(err);
    process.exit(1);
